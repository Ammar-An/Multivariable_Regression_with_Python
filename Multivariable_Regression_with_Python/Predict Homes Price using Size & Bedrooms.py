...
path2 = 'Predict Homes Price using Size & Bedrooms.txt'
...
# rescaling data (تدريج البيانات) 
# غالباً يتم القسمة على الrange بدل الstd 
...
theta2 = np.matrix(np.array([0,0,0])) # y = theta0 + theta1*x1 + theta2*x2
...
x = np.linspace(data2.Bedrooms.min(), 
data2.Bedrooms.max(), 100)
print('x \n',x) 
print('g \n',g2) 
################################# Question ###################################
############# why we didn't put g2[0, 2] , it's theta 2 
############# why we didn't use it any time befor ??!!!
f = g2[0, 0] + (g2[0, 1] * x) 
print('f \n',f) 
